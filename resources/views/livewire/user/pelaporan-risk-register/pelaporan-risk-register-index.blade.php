                        ...
                        <div class="flex flex-col">
                            ...
                            <div class="px-4 py-3 mt-4 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                                ...
                                <h2 class=" my-6 text-2xl font-semibold text-gray-700">
                                    Data Laporan Risk Register Unit
                                </h2>
                                ...
                                <button wire:click="generate_report"
                                    class="mt-5 px-4 py-2 font-medium leading-5 text-white transition-colors duration-150 bg-green-600 border border-transparent rounded-lg active:bg-green-600 hover:bg-green-700 focus:outline-none focus:shadow-outline-green"><i
                                        class="fas fa-fw fa-plus"></i>Generate Data</button>
                                <button onclick="confirmPostingAll()"
                                    class="mt-5 px-4 py-2 font-medium leading-5 text-white transition-colors duration-150 bg-sky-600 border border-transparent rounded-lg active:bg-sky-600 hover:bg-sky-700 focus:outline-none focus:shadow-outline-sky">
                                    <i class="fas fa-fw fa-save"></i> Posting Semua
                                </button>
                                ...
                                <table
                                    ...
                                    <tbody>
                                        @foreach ($data as $item)
                                            ...
                                            @else
                                                <button title="Posting Data"
                                                    onclick="confirmPosting({{ $item->id }})"
                                                    class="px-2 py-2 font-medium leading-5 text-white transition-colors duration-150 bg-green-400 border border-transparent rounded-lg active:bg-green-600 hover:bg-green-700 focus:outline-none focus:shadow-outline-green"><i
                                                        class="fas fa-fw fa-check"></i>
                                                </button>
                                        @endif
                                        <td class="px-4 py-2 border text-center whitespace-nowrap">
                                            @if ($item->posting == 'N')
                                                <button title="Validasi Data"
                                                    wire:click="validasi({{ $item->id }})"
                        ...
                        <script>
                            function confirmPosting(id) {
                                Swal.fire({
                                    ...
                                    text: 'Data akan diproses dan tidak bisa diubah lagi.',
                                    icon: 'warning',
                                    ...
                                    if (result.isConfirmed) {
                                        Livewire.emit('postingConfirmed', id);

                                        Swal.fire({
                                            icon: 'success',
                                            title: 'Diproses!',
                                            text: 'Data sedang diposting.',
                                            timer: 1500,
                            ...
                            function confirmPostingAll() {
                                Swal.fire({
                                    title: 'Yakin ingin posting semua data?',
                                    text: 'Data akan diproses dan tidak bisa diubah lagi.',
                                    icon: 'warning',
                                    showCancelButton: true,
                                    buttonsStyling: false, // Penting: agar class kustom Tailwind digunakan
                                    ...
                                    if (result.isConfirmed) {
                                        Livewire.emit('postingConfirmedAll');
