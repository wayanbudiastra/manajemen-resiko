                            ...
                            <div class="col-span-6">
                                <div class="mb-3">
                                    ...
                                    focus:text-gray-700 focus:bg-white focus:border-green-300 focus:outline-none"
                                        aria-label="Default select example" name="nama_unit"
                                        wire:model.defer="nama_unit">
                                    @error('nama_unit')
                                ...
                                <div class="mb-3">
                                    <label for="exampleText0"
                                        class="form-label inline-block mb-2 text-gray-700">Kabid</label>
                                    <select class="form-select form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0
                                        focus:text-gray-700 focus:bg-white focus:border-green-300 focus:outline-none"
                                        aria-label="Default select example" name="portal_kabid_id"
                                        wire:model.defer="portal_kabid_id">
                                        <option selected value="">-- Kabid Unit--</option>
                                        @foreach ($kabid as $item)
                                        <option value="{{$item->id}}">{{$item->nama_kabid}}</option>
                                        @endforeach

                                    </select>
                                    @error('portal_kabid_id')
                                    <div class="mb-2 text-sm text-red-700  rounded-lg dark:bg-red-200 dark:text-red-800"
                                        role="alert">
                                        <span class="font-medium"></span>{{ $message }}
                                    </div>
