...
class PelatihanPostes extends Component
{
    public $param;
    ...
    public function store()
    {
        ...
        $cek_limit = Pelatihan_user_log::where('pelatihan_user_id', $this->param)->where('kegiatan_id', '3')->max('no_limit');
        $limit = 0;
        if($cek_limit){
            $limit = $cek_limit;
        }
        $counter_limit = $limit + 1;
       
        try {
            $log = new Pelatihan_user_log();
            $log->pelatihan_user_id = $this->param;
            $log->kegiatan_id = "3";
            $log->no_limit = $limit + 1;
            $log->hasil = 0;
            $log->keterangan = "Postes ke ". $counter_limit;
            $log->save();
            ...
            $lulus = "N";
            if ($hasil_tes >=  $this->kelulusan) {
                $lulus = "Y";
                $pelatihan_user = Pelatihan_user::find($this->param);
                $pelatihan_user->update([
                    'kelulusan' => $lulus,
                    'hasil_postes' => $hasil_tes
                ]);
            }
            ...
            // dd($hasil_jawaban);
            return redirect()->to('/pelatihan-user-postes-hasil/' . $log->id);
        } catch (Exception $e) {
            dd($e);
    ...
    public function render()
    {
        ...
        return view(
            'livewire.user.pelatihan.pelatihan-postes',
            [
                "no" => 1,
                "data" => $data,
                "soal" => $soal
